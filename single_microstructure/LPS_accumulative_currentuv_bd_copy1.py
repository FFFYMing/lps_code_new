...
class FNO2d(nn.Module):
    ...
    def forward(self, x):
        ...
        return damage + 0.5 * (1.0 + self.tanh_general(x, 0.1))
...
TRAIN_PATH20 = '../LPS_data_mat/KIC_20_6_70_damagecorrect.mat'
TEST_PATH20 = '../LPS_data_mat/KIC_20_6_70_damagecorrect.mat'
...
batch_size = 5
learning_rate = 0.005 #originally it's 5e-3
...
u_train = reader.read_field('u')[start+1:ntrain+1,::r].reshape(ntrain-start,s1,s2)
v_train = reader.read_field('v')[start+1:ntrain+1,::r].reshape(ntrain-start,s1,s2)
damage_train = reader.read_field('damage')[start+1:ntrain+1,::r].reshape(ntrain-start,s1,s2,1)
dlast_train = reader.read_field('damage')[start:ntrain-step_train,::r].reshape(ntrain-start-step_train,s1,s2,1)
...
u_test = reader.read_field('u')[1+ntrain:ntrain+ntest+1,::r].reshape(ntest,s1,s2)
v_test = reader.read_field('v')[1+ntrain:ntrain+ntest+1,::r].reshape(ntest,s1,s2)
damage_test = reader.read_field('damage')[1+ntrain:1+ntrain+ntest,::r].reshape(ntest,s1,s2,1)
dlast_test = reader.read_field('damage')[ntrain:ntrain+ntest,::r].reshape(ntest,s1,s2,1)
...
#x=torch.linspace(0.005, 3.995, steps=400)
x = torch.linspace(0.005, 1.995, steps=200, device=device)
y=torch.linspace(-0.995,0.995, steps=200, device=device)
xx,yy=torch.meshgrid(x,y,indexing="ij")
x1=xx.flatten()
y1=yy.flatten()
points=torch.column_stack((x1,y1))
...
# losstest = torch_wasserstein_loss(x,y)
# xx=np.array([1,2,3,4,5])
# yy=np.array([2,3,4,5,6])
# losstesttt = wasserstein_distance(xx,yy)
...
for nlayer in range(4):  # 4
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=1e-5)  #weight_decay 1e-3, ##3e-3
    ...
    train_l2_min = 1e10
    train_l2_original_min = 1e10
    #here train_l2_min is modified to wassetstein distance (ot_loss)
    ...
    for ep in range(epochs):
        ...
        for x, dis,d in train_loader:
            ...
            for step in range(step_train):
                ...
                #loss_d = torch_wasserstein_loss(d_out.view(x.shape[0], -1), d[:, :, :, step:step + 1].view(x.shape[0], -1))
                #loss_dtest = weighted_wasserstein(d[0:1, :, :, step:step + 1].view(1, -1), d[0:1, :, :, step:step + 1].view(1, -1),points)
                loss_d = weighted_wasserstein(d_out[0:1,:,:].view(1, -1), d[0:1, :, :, step:step + 1].view(1, -1), points)
                loss_d += weighted_wasserstein(d_out[1:2,:,:].view(1, -1), d[1:2, :, :, step:step + 1].view(1, -1), points)
                loss_d += weighted_wasserstein(d_out[2:3,:,:].view(1, -1), d[2:3, :, :, step:step + 1].view(1, -1), points)
                loss_d += weighted_wasserstein(d_out[3:4,:,:].view(1, -1), d[3:4, :, :, step:step + 1].view(1, -1), points)
                loss_d += weighted_wasserstein(d_out[4:5,:,:].view(1, -1), d[4:5, :, :, step:step + 1].view(1, -1), points)

                #loss_dd = weighted_wasserstein(d_out.view(x.shape[0], -1), d[:, :, :, step:step + 1].view(x.shape[0], -1),[points,points])

                # loss_d = myloss(d_out.view(x.shape[0], -1), d[:, :, :, step:step + 1].view(x.shape[0], -1))

                # testdout=d_out[0:1, :, :].view(1, -1)
                # testd=d[0:1, :, :, step:step + 1].view(1, -1)
                #
                ...
                #
                # loss_d = myloss(d_out[0:1, :, :].view(1, -1), d[0:1, :, :, step:step + 1].view(1, -1))
                # loss_d += myloss(d_out[1:2, :, :].view(1, -1), d[1:2, :, :, step:step + 1].view(1, -1))
                # loss_d += myloss(d_out[2:3, :, :].view(1, -1), d[2:3, :, :, step:step + 1].view(1, -1))
                # loss_d += myloss(d_out[3:4, :, :].view(1, -1), d[3:4, :, :, step:step + 1].view(1, -1))
                # loss_d += myloss(d_out[4:5, :, :].view(1, -1), d[4:5, :, :, step:step + 1].view(1, -1))
                # loss_d /= 5.0

                # loss_d = torch_wasserstein_loss(d_out[0:1, 80:120, :].view(8000, -1), d[0:1, 80:120, :, step:step + 1].view(8000, -1))
                # loss_d += torch_wasserstein_loss(d_out[1:2, 80:120, :].view(8000, -1), d[1:2, 80:120, :, step:step + 1].view(8000, -1))
                # loss_d += torch_wasserstein_loss(d_out[2:3, 80:120, :].view(8000, -1), d[2:3, 80:120, :, step:step + 1].view(8000, -1))
                # loss_d += torch_wasserstein_loss(d_out[3:4, 80:120, :].view(8000, -1), d[3:4, 80:120, :, step:step + 1].view(8000, -1))
                # loss_d += torch_wasserstein_loss(d_out[4:5, 80:120, :].view(8000, -1), d[4:5, 80:120, :, step:step + 1].view(8000, -1))

                # d_outtest=d_out.view(x.shape[0], -1)
                # d_outtest0=d_outtest[0,:]
                # d_outtest1 = d_outtest[1, :]
                # d_outtest2 = d_outtest[2, :]
                # d_outtest3 = d_outtest[3, :]
                # d_outtest4 = d_outtest[4, :]
                # d_test=d[:, :, :, step:step + 1].view(x.shape[0], -1)

                loss_d_original = myloss(d_out.view(x.shape[0], -1), d[:,:,:,step:step+1].view(x.shape[0], -1))

                loss += loss_d

                loss_original += loss_d_original
            loss.backward()
            ...
            train_l2_original += loss_original.item()/step_train

        train_l2 /= ntrain
        train_l2_u /= ntrain
        train_l2_d /= ntrain
        train_l2_original /= ntrain
        model.eval()
        if train_l2 < train_l2_min:
            train_l2_min = train_l2
            train_l2_original_min = train_l2_original
            torch.save(model.state_dict(), model_filename)
            test_u_l2 = 0.0
            test_d_l2 = 0.0
            test_d_l2_original = 0.0
            with torch.no_grad():
                with torch.no_grad():
                    for step in range(ntest):
                        ...
                        test_d_l2 += weighted_wasserstein(im_d.view(x.shape[0], -1), d.view(x.shape[0], -1),points)
                        #print(im_d)
                        #print(myloss(im_d.view(x.shape[0], -1), d.view(x.shape[0], -1)))

                        test_d_l2_original += myloss(im_d.view(x.shape[0], -1), d.view(x.shape[0], -1))
                test_u_l2 /= ntest
                test_d_l2 /= ntest
                test_d_l2_original /= ntest
        ...
        print("depth: %d, epoch: %d, current training l2: %f,best training l2: %f, current training wass: %f,best wass %f, training u: %f training d:%f test_u_l2: %f, test_d_wass: %f, test_d_l2: %f" %(nb, ep, train_l2_original, train_l2_original_min, train_l2, train_l2_min, train_l2_u, train_l2_d,test_u_l2,test_d_l2, test_d_l2_original))
