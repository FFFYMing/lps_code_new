...
class FNO2d(nn.Module):
    def __init__(self, modes1, modes2,  width, nlayer):
        ...
        self.convlayer11 = nn.ModuleList([SpectralConv2d(self.width, self.width, self.modes1, self.modes2).cuda() for i in range(self.nlayer)]) 
        self.w11 = nn.ModuleList([nn.Conv1d(self.width, self.width, 1).cuda() for i in range(self.nlayer)]) 
    ...
    def forward(self, x):
        ...
        for layer in range(self.nlayer-1):
            u11_1 = self.convlayer11[layer](u11)
            u11_2 = self.w11[layer](u11.view(batchsize, self.width, -1)).view(batchsize, self.width, size_x, size_y)
            u11 = F.relu(u11_1 + u11_2)

        u11_1 = self.convlayer11[self.nlayer-1](u11)
        u11_2 = self.w11[self.nlayer-1](u11.view(batchsize, self.width, -1)).view(batchsize, self.width, size_x, size_y)
        u11 = (u11_1 + u11_2)
...
base_dir = './LPS_FNO/currentuv_bd_seed1513'
...
optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=3e-3)
...
np.savetxt('fno_damage.txt',np.reshape(solution,(ntest,-1)))
"""
