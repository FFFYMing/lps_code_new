...
base_dir = './LPS_FNO_NKN_wwass_m20_w48_lr5e-3_wdc1e-5_0415/shatodeep_NKN_currentuv_bd'
...
test_u_l2 = 0.0
test_d_l2 = 0.0
sol = torch.zeros((ntest,s,s))


for nlayer in range(1,2):
    nb = 2**nlayer
    print("nlayer: %d" % nb)
    model = FNO2d(modes, modes, width, nb).cpu()
    ...
    t2 = default_timer()
    print("depth: %d, epoch: %d, current training l2: %f,best training l2: %f, training u: %f training d:%f test_u_l2: %f, test_d_l2: %f" %(nb, 1,0.0,train_l2_min, 0.0, 0.0,test_u_l2,test_d_l2))


solution = sol.detach().cpu().numpy()
gt = gt.detach().cpu().numpy()

np.savetxt('gt_damage.txt',np.reshape(gt,(ntest,-1)))
np.savetxt('ifno_damage.txt',np.reshape(solution,(ntest,-1)))
...
for i in range(3):
    img1 = ax[i,0].imshow(gt[int(ind[i]),:,:,0])
    fig.colorbar(img1,ax = ax[i,0],shrink = 0.6)
    img2 = ax[i, 1].imshow(solution[int(ind[i]), :, :])
    fig.colorbar(img2,ax = ax[i,1],shrink = 0.6)
    img5 = ax[i, 2].imshow(np.sqrt(np.power(gt[int(ind[i]), :, :,0] - solution[int(ind[i]),:, :],2)))
    fig.colorbar(img5, ax=ax[i, 2], shrink=0.6)
    ...
    if i == 0:
        ax[i, 0].set_title('Ground Truth d', fontsize=10)
        ax[i, 1].set_title('Approximation d', fontsize=10)
        ax[i, 2].set_title('Error',fontsize = 10)
    ax[i, 0].set_xticks([])
    ax[i, 0].set_yticks([])
    ax[i, 1].set_xticks([])
    ax[i, 1].set_yticks([])
